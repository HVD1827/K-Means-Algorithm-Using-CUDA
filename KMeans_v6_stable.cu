...
__global__ void update_centroids(int *in_cluster, float *dist_points, 
                                ...
                                int N, int d, int k)
{
    ...
    dist_points[cluster*(d*N) + dim_idx*N + point_idx] = data[point_idx * d + dim_idx]; //ulta store kr raha

    if(dim_idx == 0)
    {
        in_cluster[N* cluster + point_idx] = 1;
    }
}

void compute_centroids(int *in_cluster, float *dist_points, int i, float *centroids, int *counts,
    int N, int d, int k)
{
    int cluster = i / d;
    int dim = i % d;
...
__global__ void compute_probabs_2(int *in_centroids_set, int *in_centroids_set_flag,
                                  int *centroid_set, int *centroid_count, int N)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= N)
        return;
    ...
    {
        centroid_set[*centroid_count + in_centroids_set[idx] - 1] = idx;
    }
}
...
int main()
{
    ...
    for (; iter < max_iters; ++iter)
    {
        ...
        int *d_in_cluster; 
        cudaMalloc(&d_in_cluster, N*k*sizeof(int));
        float *d_dist_points;
        cudaMalloc(&d_dist_points, k*N*d*sizeof(float));

        update_centroids<<<(N * d + 255) / 256, 256>>>(
            d_in_cluster, d_dist_points, d_data, d_labels, N, d, k);

        cudaDeviceSynchronize();

        // cudaStream_t streams[k*d];
        ...
        for (int i = 0; i < k*d; ++i)
        {
            ...
            compute_centroids(d_in_cluster + N*cluster, d_dist_points + d*N*cluster + dim*N, i, d_centroids, counts, N, d, k);
        }
