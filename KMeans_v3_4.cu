...
__global__ void compute_distances_kernel(float *data, float *centroids,
                                ...
                                int l, int N)
{
    ...
    probabilities[idx] = (float)l * (min_distances[idx] / *total_cost);
}

// Optimized reduction-based compute_total_cost kernel
__global__ void compute_total_cost(float *min_distances, float *total_cost, int N)
{
    ...
    if (i < N)
    {
        shmem[threadIdx.x] = min_distances[i];
    }
    else
    {
...
int main()
{
    ...
    for (int r = 0; r < rounds; ++r)
    {
        ...
        compute_total_cost<<<((N) + 255), 256, 256 * sizeof(float)>>>(
            d_min_distances, d_total_cost, N);

        ERROR(3);
        cudaDeviceSynchronize();

        compute_probabs<<<((N) + 255) / 256, 256>>>(d_min_distances, d_probabs, d_total_cost, l_kmp, N);
        ERROR(4);
        cudaDeviceSynchronize();
        ...
        for (int i = 0; i < N; ++i)
        {
            float probab_random = (float)rand() / ((float)RAND_MAX);
            if (probab_random < h_probabs[i])
            {
                C_indices_kmp.insert(i);
            }
        }
    }
    cout << C_indices_kmp.size() << endl;
    for (auto it : C_indices_kmp)
    {
        cout << it << " ";
    }
    cout << endl;
    ...
    for (auto it : C_indices_kmp)
    {
        if (cnt == k)
            break;
        cnt++;
        assign_centroids<<<(d + 255) / 256, 256>>>(d_data, d_centroids, d, it, cnt);
    }
    ...
    {
        compute_distances_kernel<<<(N * k + 255) / 256, 256>>>(
