                                  ...
                                  float *distances, int N, int d, int k)
{
    ...
    int point_idx = idx % N;
    int centroid_idx = idx / N;

    float sum = 0.0f;
    for (int i = 0; i < d; ++i)
    {
        float diff = data[point_idx * N + i] - centroids[centroid_idx * d + i];
        sum += diff * diff;
    }
    distances[idx] = sum;
}
                                      ...
                                      float *distances, int *centroid_set, int N, int d, int size, int k)
{
    ...
    int point_idx = idx % N;
    int centroid_idx = centroid_set[idx / N];

    float sum = 0.0f;
    for (int i = 0; i < d; ++i)
    {
        float diff = data[point_idx + i * N] - data[centroid_idx + i * N];
        sum += diff * diff;
    }
    distances[idx] = sum;
}
                                ...
                                float *min_distances, int size, int N, int k)
{
    ...
    float minm_dist = distances[idx];
    int minm_index = 0;

    for (int j = 1; j < size; ++j)
    {
        float dist = distances[idx + j * N];
        if (dist < minm_dist)
        {
            minm_dist = dist;
            minm_index = j;
                                 ...
                                 int N, int d, int k)
{
    ...
    atomicAdd(&centroids[cluster * d + dim_idx], data[point_idx + dim_idx * N]);
    if (dim_idx == 0)
    {
...
__global__ void assign_centroids_fast(float *data, float *centroids, int d, int *centroid_set, int centroid_indx, int N)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= d)
        return;
    centroids[(centroid_indx * d) + idx] = data[centroid_set[centroid_indx] + N * idx];
}
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < k; i++)
    {
        assign_centroids_fast<<<(d + 255) / 256, 256>>>(d_data, d_centroids, d, d_centroid_set, i, N);
    }
