...
__global__ void compute_distances(float *data, float *centroids,
                                  float *distances, int N, int d, int k, int j)
{
    ...
    distances[point_idx * k + centroid_idx] = sum;
}

__global__ void compute_distances_kmp(float *data,
                                      float *distances, int *centroid_set, int N, int d, int size, int k, int j)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= N)
        return;
    ...
    int point_idx = idx;
    int centroid_idx = centroid_set[j];
...
int main()
{
    ...
    cudaStream_t streams_kmp[k];
    for (int j = 0; j < k; j++)
    {
        cudaStreamCreate(&streams_kmp[j]);
    }
    for (int r = 0; r < rounds; ++r)
    {
        ...
        {
            compute_distances_kmp<<<(N + 255) / 256, 256, 0, streams_kmp[j]>>>(
                d_data, d_distances_kmp, d_centroid_set, N, d, size, k, j);
        }
    ...
    for (; iter < max_iters; ++iter)
    {
        for (int j = 0; j < k; j++)
        {
            compute_distances<<<(N + 255) / 256, 256, 0, streams[j]>>>(
                d_data, d_centroids, d_distances, N, d, k, j);
            ERROR(5);
        }
